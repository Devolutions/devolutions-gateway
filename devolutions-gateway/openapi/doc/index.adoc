...
| 
| probe
| The probes to run. Each value is either `ping`, a port number (`22`), or a named service (`rdp`, `https`, ΓÇª). Validation is deferred to scan-time so failures can be surfaced as a structured 400 ΓÇö naming the offending value ΓÇö instead of a generic serde rejection at extraction time. <<String>>
| -
| null
| 
| enable_ping_start
