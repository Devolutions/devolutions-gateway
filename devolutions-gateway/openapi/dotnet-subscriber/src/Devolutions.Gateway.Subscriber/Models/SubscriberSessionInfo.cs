/*
 * devolutions-gateway-subscriber
 *
 * API a service must implement in order to receive Devolutions Gateway notifications
 *
 * The version of the OpenAPI document: 2024.1.3
 * Contact: infos@devolutions.net
 * Generated by: https://openapi-generator.tech
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Devolutions.Gateway.Subscriber.Converters;

namespace Devolutions.Gateway.Subscriber.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public class SubscriberSessionInfo : IEquatable<SubscriberSessionInfo>
    {
        /// <summary>
        /// Gets or Sets AssociationId
        /// </summary>
        [Required]
        [DataMember(Name="association_id", EmitDefaultValue=false)]
        public Guid AssociationId { get; set; }

        /// <summary>
        /// Gets or Sets StartTimestamp
        /// </summary>
        [Required]
        [DataMember(Name="start_timestamp", EmitDefaultValue=false)]
        public DateTime StartTimestamp { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class SubscriberSessionInfo {\n");
            sb.Append("  AssociationId: ").Append(AssociationId).Append("\n");
            sb.Append("  StartTimestamp: ").Append(StartTimestamp).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (obj is null) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((SubscriberSessionInfo)obj);
        }

        /// <summary>
        /// Returns true if SubscriberSessionInfo instances are equal
        /// </summary>
        /// <param name="other">Instance of SubscriberSessionInfo to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(SubscriberSessionInfo other)
        {
            if (other is null) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    AssociationId == other.AssociationId ||
                    AssociationId != null &&
                    AssociationId.Equals(other.AssociationId)
                ) && 
                (
                    StartTimestamp == other.StartTimestamp ||
                    StartTimestamp != null &&
                    StartTimestamp.Equals(other.StartTimestamp)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (AssociationId != null)
                    hashCode = hashCode * 59 + AssociationId.GetHashCode();
                    if (StartTimestamp != null)
                    hashCode = hashCode * 59 + StartTimestamp.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(SubscriberSessionInfo left, SubscriberSessionInfo right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(SubscriberSessionInfo left, SubscriberSessionInfo right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
