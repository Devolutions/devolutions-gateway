...
use devolutions_gateway::agent_tunnel::registry::{AGENT_OFFLINE_TIMEOUT, AgentPeer, AgentRegistry};
...
fn update_routes_new_epoch_replaces() {
    ...
    let subnet: IpNetwork = "10.0.0.0/8".parse().expect("valid CIDR");
    ...
    let new_subnet: IpNetwork = "192.168.0.0/16".parse().expect("valid CIDR");
...
fn update_routes_same_epoch_refreshes_only() {
    ...
    let different_subnet: IpNetwork = "172.16.0.0/12".parse().expect("valid CIDR");
...
fn update_routes_stale_epoch_ignored() {
    ...
    let old_subnet: IpNetwork = "172.16.0.0/12".parse().expect("valid CIDR");
