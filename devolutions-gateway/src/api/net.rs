...
use network_scanner::sources::{NetworkScanSourceProvider, ScannerSource, ScannerSourceCapabilities};
...
pub async fn handle_network_scan(
    ...
    let source_inventory = network_scanner::sources::SystemNetworkScanSourceProvider
    ...
    let scan_plan = match plan_scan(
        ...
        Ok(plan) => plan,
        Err(error) => return Ok(network_scan_plan_error_response(error)),
    };
    let scan_sources = scan_plan.sources;
        ...
        loop {
            tokio::select! {
                result = receiver.recv() => {
                    ...
                    let Some(response) = filter.serialize_event(event.0, &scan_sources) else {
                        continue;
                    };
...
pub enum Probe {
    Ping,
    /// TCP port number or known service name.
    Port(#[cfg_attr(feature = "openapi", schema(value_type = String))] MaybeNamedPort),
}
...
impl TryFrom<NetworkScanQueryParams> for (NetworkScannerParams, ScanEventFilter) {
    ...
    fn try_from(val: NetworkScanQueryParams) -> Result<Self, Self::Error> {
        ...
        target_selector.validate(DEFAULT_MAX_TARGET_RANGE_ADDRESSES)?;
        ...
        let probe: Vec<Probe> = match (val.enable_tcp_probes, val.probes.len()) {
            (false, _) => Vec::new(),
            (true, 0) => COMMON_PORTS.iter().map(|port| Probe::Port((*port).into())).collect(),
            (true, _) => val.probes,
        };

        let has_ping_probe = probe.iter().any(|probe| matches!(probe, Probe::Ping));
        #[allow(deprecated)]
...
pub(crate) async fn get_net_config(_token: crate::extract::NetScanToken) -> Result<Response, HttpError> {
    ...
    // RFC 8594 deprecation triple: signal that this endpoint is on its way
    // out, point clients to the v2 alternative, and (intentionally) leave
    // the `Sunset` date open until product confirms a removal milestone.
    // `Deprecation: true` + `Link: rel="successor-version"` is enough to
    // surface the migration in generated clients today.
...
mod tests {
    ...
    fn query_mapping_disables_tcp_probes() {
        let (scanner_params, _) = NetworkScanQueryParams {
            ...
            probes: vec![Probe::Port(MaybeNamedPort::Port(3389))],
            ..minimal_query()
        }
        .try_into()
    ...
    fn query_mapping_rejects_mixed_ip_family_targets_with_ranges() {
        ...
        assert!(error.to_string().contains("mixes IPv4 and IPv6"));
    }
