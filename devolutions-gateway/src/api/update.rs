...
pub(super) async fn trigger_update_check(
    ...
    if let Some(v) = query_version {
        ...
        request
            .products
            .insert(UpdateProduct::Gateway, UpdateProductInfo { version });
    }
    ...
    if request.products.is_empty() {
        return Err(
            HttpErrorBuilder::new(StatusCode::BAD_REQUEST).msg("request must specify at least one product to update")
        );
    }
...
pub(super) async fn get_update_products(_scope: UpdateReadScope) -> Result<Json<GetUpdateProductsResponse>, HttpError> {
    ...
    let products = status
        .into_products()
        .into_iter()
        .map(|(k, v)| (UpdateProduct::from(k), UpdateProductInfo { version: v.version }))
        .collect();
