...
async fn spawn_tasks(conf_handle: ConfHandle) -> anyhow::Result<Tasks> {
    ...
    let authorization_store: agent_tunnel::authorization::DynAgentAuthorizationStore = Arc::new(authorization_store);
    ...
    let state = DgwState {
        ...
        agent_tunnel_handle: Some(Arc::new(agent_tunnel_handle)),
    };
