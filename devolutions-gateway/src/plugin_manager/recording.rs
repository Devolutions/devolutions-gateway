...
use std::{sync::Arc, ffi::CString, os::raw::c_char};
...
pub struct RecordingApi<'a> {
    ...
    NowRecording_UpdateRecording: Symbol<'a,unsafe extern "C" fn(ctx: RecordingContext, buffer: *mut u8, updateX: u32, updateY: u32, updateWidth: u32, updateHeight:u32, surfaceStep: *const u32)>,
...
impl Recorder {
    pub fn new(lib: Arc<Library>) -> Self {
        ...
        let ctx = unsafe {
            (api.NowRecording_CreateRecordingContext)()
        };
    ...
    pub fn update_recording(&self, mut image_data: ImageUpdate) {
        unsafe {
            (self.api.NowRecording_UpdateRecording)(self.ctx, image_data.image_buff.as_mut_ptr(),
                                                    image_data.update_x, image_data.update_y, image_data.update_width,
                                                    image_data.update_height, (&mut image_data.surface_step) as *mut u32);
        }
    }
