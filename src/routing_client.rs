...
impl Client {
    ...
    pub fn serve<T: 'static + Transport + Send>(self, client_transport: T) -> Box<Future<Item = (), Error = io::Error> + Send> {

        let server_conn = TcpTransport::connect(&self.routing_url);

        Box::new(
            server_conn.and_then(move |server_transport| {
        ...
        //Box::new(jet_stream_client.forward(jet_sink_client))

//        Box::new(tls_acceptor.accept(conn).and_then(|tlsStream| {
//            let transport = TlsTransport::new(tlsStream);
//            ok(())
//        }))

//        Box::new(TcpTransport::connect(&self.routing_url).and_then(move |transport| {
//
//
//            let jet_sink_server = transport.message_sink();
//            let jet_stream_server = transport.message_stream();
//
//            let client_transport = TcpTransport::new(conn);
//            let jet_sink_client = client_transport.message_sink();
//            let jet_stream_client = client_transport.message_stream();
//
//            // Build future to forward all bytes
