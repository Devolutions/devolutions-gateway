...
export class AgentEnrollmentComponent extends BaseComponent implements OnInit {
    ...
    this.apiService
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.enrollmentError = err?.status === 404
            ? 'Agent tunnel endpoint not available. Ensure the gateway is running with agent tunnel enabled.'
            : `Failed to generate enrollment command: ${err?.statusText || err?.message || 'Unknown error'}`;
          this.generating = false;
        },
      });
  }
  ...
  humanizeExpiry(unixTimestamp: number): string {
