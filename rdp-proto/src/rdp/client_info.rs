...
impl PduParsing for ClientInfo {
    ...
    fn to_buffer(&self, mut stream: impl io::Write) -> Result<(), Self::Error> {
        ...
        write_string_with_null_terminator(&mut stream, self.credentials.username.as_str(), character_set)?;
        write_string_with_null_terminator(&mut stream, self.credentials.password.as_str(), character_set)?;
        write_string_with_null_terminator(&mut stream, self.alternate_shell.as_str(), character_set)?;
...
impl PduParsing for TimezoneInfo {
    ...
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, Self::Error> {
        ...
        let standard_name = read_string(&mut stream, TIMEZONE_INFO_NAME_LEN, CharacterSet::Unicode, false)?;
        let standard_date = SystemTime::from_buffer(&mut stream)?;
        let standard_bias = stream.read_u32::<LittleEndian>()?;

        let daylight_name = read_string(&mut stream, TIMEZONE_INFO_NAME_LEN, CharacterSet::Unicode, false)?;
...
impl PduParsing for SystemTime {
    ...
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, Self::Error> {
        ...
        let month = Month::from_u16(stream.read_u16::<LittleEndian>()?).ok_or(ClientInfoError::InvalidSystemTime)?;
        let day_of_week =
            DayOfWeek::from_u16(stream.read_u16::<LittleEndian>()?).ok_or(ClientInfoError::InvalidSystemTime)?;
...
impl_from_error!(std::string::FromUtf8Error, ClientInfoError, ClientInfoError::Utf8Error);
