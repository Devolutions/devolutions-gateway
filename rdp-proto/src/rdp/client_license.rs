...
impl PduParsing for ClientLicense {
    ...
    fn to_buffer(&self, mut stream: impl io::Write) -> Result<(), Self::Error> {
        self.preamble.to_buffer(&mut stream, self.buffer_length() as u16)?;
...
impl LicensePreamble {
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, ClientLicenseError> {
        let message_type = PreambleType::from_u8(stream.read_u8()?).ok_or(ClientLicenseError::InvalidLicenseType)?;
        ...
        let flags = PreambleFlags::from_bits(flags_with_version & !PROTOCOL_VERSION_MASK)
            .ok_or_else(|| ClientLicenseError::InvalidPreamble(String::from("Got invalid flags field")))?;
        let version =
            PreambleVersion::from_u8((flags_with_version & PROTOCOL_VERSION_MASK) as u8).ok_or_else(|| {
                ClientLicenseError::InvalidPreamble(String::from("Got invalid version in the flags field"))
            })?;
    ...
    fn to_buffer(&self, mut stream: impl io::Write, message_size: u16) -> Result<(), ClientLicenseError> {
...
impl PduParsing for LicensingBinaryBlob {
    ...
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, Self::Error> {
        let blob_type =
            BlobType::from_u16(stream.read_u16::<LittleEndian>()?).ok_or(ClientLicenseError::InvalidBlobType)?;
