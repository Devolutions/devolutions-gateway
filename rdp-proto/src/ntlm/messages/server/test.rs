...
const LOCAL_SESSION_KEY: [u8; 16] = [
    0x6, 0x57, 0xf2, 0xe4, 0x66, 0x52, 0xca, 0x92, 0x2, 0x88, 0x6d, 0xe7, 0x74, 0xf1, 0x9b, 0x4,
];
...
fn read_authenticate_local_logon_correct_reads_encrypted_random_session_key() {
    let buffer = LOCAL_AUTHENTICATE_MESSAGE;
    let expected = LOCAL_SESSION_KEY;
    let mut context = Ntlm::new(get_test_identity(), NTLM_VERSION);
    context.negotiate_message = Some(NegotiateMessage::new(Vec::new()));
    context.challenge_message = Some(ChallengeMessage::new(
        vec![0x04, 0x05, 0x06],
    ...
    assert_eq!(
        expected.as_ref(),
        context.authenticate_message.unwrap().session_key.as_ref()
    );
}
...
fn read_authenticate_local_logon_correct_reads_user_name() {
    ...
    assert_eq!(expected.as_ref(), context.identity.user.as_slice());
}
...
fn read_authenticate_domain_logon_correct_reads_domain_name() {
    ...
    assert_eq!(expected.as_ref(), context.identity.domain.as_slice());
}
...
fn complete_authenticate_does_not_fail_on_correct_mic() {
    ...
    context.authenticate_message = Some(AuthenticateMessage::new(
        ...
        Some(Mic::new(
            [
                0xfb, 0x11, 0x59, 0x36, 0x5, 0x78, 0x33, 0x22, 0x1f, 0x20, 0x20, 0x37, 0x5c, 0x20, 0x40, 0xc9,
            ],
            64,
        )),
        [
