...
impl PduParsing for ClientGccBlocks {
    ...
    fn from_buffer(mut buffer: impl io::Read) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            network: network.ok_or(GccError::RequiredClientDataBlockIsAbsent(ClientGccType::NetworkData))?,
            cluster,
            monitor,
            message_channel,
    ...
    fn to_buffer(&self, mut buffer: impl io::Write) -> Result<(), Self::Error> {
        ...
        UserDataHeader::from_gcc_block(ClientGccType::NetworkData, &self.network)?.to_buffer(&mut buffer)?;
