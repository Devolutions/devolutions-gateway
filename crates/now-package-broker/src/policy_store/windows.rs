...
/// decides, so a `.yaml`/`.yml` (or extensionless) configured path is rejected
...
/// These handles block path-component replacement, and the hosting handle anchors relative transaction names.
...
/// The existing path through ProgramData is verified and retained before creation.
/// The runtime creates only the fixed `Devolutions` and `PackageBroker` components.
///
...
///
/// Returns the retained directory, canonical path, ancestor-security digest, and retained lexical ancestors.
fn ensure_default_directory_secured(dir: &Path) -> anyhow::Result<(File, PathBuf, [u8; 32], Vec<File>)> {
    ...
    let mut ancestor_handles =
        policy_security::retain_policy_no_reparse_directory_chain(program_data, "ProgramData directory")?;
    let program_data_handle = ancestor_handles.pop().context("ProgramData directory chain is empty")?;
    let canonical_program_data = policy_security::final_path_from_handle(&program_data_handle)?;
    ensure!(
...
///
/// Returns the retained directory, canonical path, ancestor-security digest, and retained lexical ancestors.
fn verify_custom_directory_secure(dir: &Path) -> anyhow::Result<(File, PathBuf, [u8; 32], Vec<File>)> {
    let mut ancestor_handles =
        policy_security::retain_policy_no_reparse_directory_chain(dir, "configured policy directory")?;
...
impl DiskFingerprint {
    fn target_state(&self) -> Option<(FileIdentity, [u8; 32], [u8; 32])> {
        match self {
            Self::Active {
                target,
                content_digest,
...
impl DiskFingerprint {
    ...
    pub(super) fn test_invalid(
        ...
        Self::Invalid {
            ...
            reason: format!("{:?}", validation::DiskFailureReason::MalformedContent),
        }
    }
}
...
pub(super) fn unavailable_fingerprint(path: PathBuf) -> DiskFingerprint {
    DiskFingerprint::Invalid {
        ...
        reason: format!("{:?}", validation::DiskFailureReason::WatcherUnavailable),
    }
}
...
impl RetainedPolicyFile {
    pub(super) fn verify_matches(&self, expected: &DiskFingerprint) -> anyhow::Result<()> {
        ...
        let (expected_target, expected_content, expected_security) = expected
            .target_state()
            .context("write observation did not retain a complete target fingerprint")?;
        ...
        policy_security::verify_managed_policy_file_security(handle)
...
fn open_policy_file(path: &Path, retain_for_write: bool) -> std::io::Result<OpenedPolicyFile> {
    if retain_for_write {
        ...
        {
            Ok(file) => {
                return Ok(OpenedPolicyFile {
                    file,
                    retained_for_write: true,
                });
            }
            ...
            Err(error) => {
                tracing::warn!(
                    path = %path.display(),
                    %error,
                    "Failed to retain the configured policy file for conditional publication"
                );
            }
        }
    }
...
/// (Windows filesystems are case-insensitive but case-preserving) is accepted as the same file.
/// Security is verified on the target's open handle before any content is trusted, and content is
...
fn observe_impl(
    ...
    if let Err(error) = policy_security::verify_policy_file_path(&file, &canonical_path) {
        tracing::warn!(
            path = %canonical_path.display(),
            error = %format!("{error:#}"),
            "Configured policy file failed path verification"
        );
        return invalid_observation(
            &canonical_path,
            validation::DiskFailureReason::InsecureStorage,
    ...
    if let Err(security_error) = policy_security::verify_managed_policy_file_security(&file) {
        // Fail closed without ever reading content past a failed security check:
...
impl TransactionMarker {
    fn from_observation(
        ...
        let (old_identity, old_content_digest, old_security_digest) = expected
            .target_state()
            .context("replacement requires a complete observed target fingerprint")?;
        Ok(Self {
            id,
...
fn conditional_replace(
    ...
    if let Err(error) = temp_file
        ...
        .and_then(|()| {
            policy_security::verify_managed_policy_file_security(&temp_file)
...
impl RenameInformation {
    #[expect(
        ...
        reason = "initializing one variable-length Win32 structure is one logical unsafe operation"
    )]
    fn new(root: HANDLE, new_name: &OsStr) -> Self {
        ...
        // SAFETY: The aligned buffer is sized for FILE_RENAME_INFO plus the complete UTF-16 name.
        unsafe {
            ...
            std::ptr::copy_nonoverlapping(name.as_ptr(), info.cast::<u8>().add(name_offset).cast(), name.len());
        }
        Self {
            buffer,
            length: u32::try_from(buffer_len).expect("rename buffer length fits u32"),
        }
    }
...
fn set_file_rename_information(file: &File, information: &RenameInformation) -> std::io::Result<()> {
    // SAFETY: `information` contains a valid variable-length FILE_RENAME_INFO buffer.
    let result = unsafe {
    ...
    match result {
        Ok(()) => Ok(()),
        Err(error) if error.code() == windows::Win32::Foundation::E_INVALIDARG => {
            // SAFETY: The same validated buffer is accepted by the older information class.
            unsafe {
                SetFileInformationByHandle(
                    HANDLE(file.as_raw_handle()),
                    FileRenameInfo,
...
fn prepublication_failure_after_cleanup(
    file: &File,
    error: anyhow::Error,
    cleanup_failure_message: &str,
) -> WriteFailure {
    match delete_file_handle(file) {
        Ok(()) => WriteFailure::PrePublication(error),
        Err(cleanup_error) => {
            WriteFailure::PrePublication(error.context(format!("{cleanup_failure_message}: {cleanup_error:#}")))
        }
    }
}
...
fn recover_create_temporary_files(dir_path: &Path) -> anyhow::Result<()> {
    ...
    for entry in std::fs::read_dir(dir_path).context("failed to enumerate policy directory for create recovery")? {
        ...
        policy_security::verify_managed_policy_file_security(&file)
            .context("policy create remnant security is invalid")?;
...
fn recover_interrupted_transaction(dir: &File, dir_path: &Path, final_leaf: &OsStr) -> anyhow::Result<()> {
    ...
    policy_security::verify_managed_policy_file_security(&marker_file)
        .context("transaction marker security is invalid")?;
...
pub(super) fn atomic_create(
    ...
    if let Err(error) = temp_file
        .write_all(bytes)
        .and_then(|()| temp_file.sync_all())
        .context("failed to persist new policy")
        .and_then(|()| {
            policy_security::verify_managed_policy_file_security(&temp_file)
                .context("new policy temporary file failed security verification")
        })
    {
...
/// The returned [`PersistedPolicy`] reflects the object made active by the handle-relative rename.
/// This check repeats committed-policy validation instead of trusting the earlier draft validation.
///
fn verify_persisted_handle(
    ...
    policy_security::verify_managed_policy_file_security(final_file)
    ...
    let persisted = read_file_from_start(final_file).context("failed to re-read persisted policy file")?;
...
fn move_replace(from: &Path, to: &Path) -> anyhow::Result<()> {
    let from = U16CString::from_os_str(from.as_os_str()).context("replacement path contains an interior NUL")?;
    let to = U16CString::from_os_str(to.as_os_str()).context("target path contains an interior NUL")?;
    // SAFETY: Both paths are valid, NUL-terminated UTF-16 strings live for the call.
    unsafe { MoveFileExW(from.as_pcwstr(), to.as_pcwstr(), MOVEFILE_REPLACE_EXISTING) }
        .context("MoveFileExW replacement failed")
}
...
mod tests {
    ...
    fn native_relative_handle_rename_uses_retained_directory_root() {
        ...
        let information = RenameInformation::new(HANDLE(dir_file.as_raw_handle()), destination.file_name().unwrap());
        nt_set_file_rename_information(&source_file, &information).unwrap();
    ...
    #[test]
    fn ordinary_reader_blocks_retention_but_not_read_observation() {
        let dir = temp_dir();
        let path = dir.path().join("policy.json");
        ...
        let ordinary_observation = open_policy_file(&path, false).unwrap();
        assert!(!ordinary_observation.retained_for_write);
    ...
    fn interrupted_repair_preserves_invalid_original_and_retires_prepublication_remnants() {
        ...
        let original = b"malformed policy";
        let dir = temp_dir();
        let dir_file = open_directory_no_reparse(dir.path()).unwrap();
        let final_path = dir.path().join("policy.json");
        let mut final_file = match create_secure_transaction_file(&final_path) {
    ...
    fn transaction_marker_round_trips_exact_state() {
        ...
        let mut older_version: serde_json::Value = serde_json::from_slice(&marker.to_bytes()).unwrap();
        older_version["Version"] = 1.into();
        assert!(TransactionMarker::from_bytes(&serde_json::to_vec(&older_version).unwrap()).is_err());

        older_version["Version"] = 2.into();
        assert!(TransactionMarker::from_bytes(&serde_json::to_vec(&older_version).unwrap()).is_err());

        let mut missing_identity = older_version;
