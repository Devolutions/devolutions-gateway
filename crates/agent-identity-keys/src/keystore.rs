...
mod tests {
    ...
    #[expect(
        clippy::print_stdout,
        reason = "A3 spike outcomes and leftover keys must be visible in CI"
    )]
    fn spike_system_only_key_inspectable_by_owner() -> anyhow::Result<()> {
        if !elevated()? {
            println!("SKIP: A3 spike requires elevation");
            return Ok(());
        }
        let user = Token::current_process_token()
            .sid_and_attributes()
            ...
            .to_string();
        if user == "S-1-5-18" {
            println!("SKIP: A3 spike requires an elevated non-SYSTEM user");
            return Ok(());
        }
        let name = crate::new_key_name("DevolutionsAgentTest-A3-");
        let provider = Provider::open()?;
        let key = provider.create(&name)?;
        if let Err(error) = configure_and_finalize(&key, &user, false) {
            let cleanup = key.delete();
            if let Err(cleanup_error) = cleanup {
                println!("A3-SPIKE: leftover={name} cleanup_error={cleanup_error}");
            }
            return Err(error);
        }

        let read = read_dacl(&key);
        let read_status = match &read {
            Ok(_) => String::from("ok"),
