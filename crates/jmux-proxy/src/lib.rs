...
type ErasedStream = Box<dyn AsyncStream + Unpin + Send + Sync>;
                    ...
                    match TcpStream::connect(socket_addr).await {
                        Ok(stream) => {
                            ...
                            let stream: ErasedStream = if let Some(interceptor) = stream_interceptor {
                                let (jmux_stream, intercepted_stream) = tokio::io::duplex(64 * 1024);
                                interceptor(destination_url.clone(), intercepted_stream, stream);
                                Box::new(jmux_stream)
                            } else {
                                Box::new(stream)
                            };
