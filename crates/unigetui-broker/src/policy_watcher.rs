    ...
    pub async fn watch(self, shutdown: Arc<Notify>) {
        ...
        let _watcher_handle = tokio::task::spawn_blocking(move || {
            ...
            watcher
                ...
                .expect("failed to watch policy directory");

            // Keep the watcher alive until the channel is closed.
            // The channel is closed when the receiver is dropped (on shutdown).
            loop {
            ...
            tokio::select! {
                ...
                Some(()) = fs_rx.recv() => {
                    ...
                    match policy_loader::load_policy(&path) {
                        ...
                        Err(e) => {
                            tracing::warn!(error = %e, "Policy reload failed; broker paused");
