...
pub fn load_policy(path: &Path) -> anyhow::Result<PolicyDocument> {
    ...
    let policy: PolicyDocument = serde_json::from_str(&content)
        .map_err(|e| anyhow::anyhow!("failed to parse policy file at {}: {e}", path.display()))?;

    // Validate basic shape.
    crate::evaluator::validate_policy_basics(&policy)?;
