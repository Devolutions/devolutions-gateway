...
impl Into<Vec<u8>> for Icmpv6Message {
    fn into(self) -> Vec<u8> {
        ...
        match self {
            ...
            Icmpv6Message::ParameterProblem { code, pointer, original_packet } => {
                bytes.push(code as u8);
                bytes.extend(vec![0; 2]); // checksum placeholder
                bytes.extend(pointer.to_be_bytes());
                bytes.extend(original_packet);
            }
            Icmpv6Message::EchoRequest { identifier, sequence_number, payload } => {
                bytes.push(0); // code
                bytes.extend(vec![0; 2]); // checksum placeholder
            ...
            Icmpv6Message::EchoReply { identifier, sequence_number, payload } => {
                bytes.push(0); // code
                bytes.extend(vec![0; 2]); // checksum placeholder
                bytes.extend(identifier.to_be_bytes());
                bytes.extend(sequence_number.to_be_bytes());
