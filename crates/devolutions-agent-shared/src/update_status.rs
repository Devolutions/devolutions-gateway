...
use crate::{
    InstalledProductUpdateInfo, UPDATE_MANIFEST_V2_MINOR_VERSION, UpdateProductKey, UpdateSchedule, VersionMajorV2,
};
...
///   "Products": { "Agent": { "Version": "2026.2.0" } }
/// }
/// ```
///
...
pub struct UpdateStatusV2 {
    ...
    ///
    /// Products that are not installed are omitted.
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub products: HashMap<UpdateProductKey, InstalledProductUpdateInfo>,
}
...
impl UpdateStatus {
    ...
    pub fn into_products(self) -> HashMap<UpdateProductKey, InstalledProductUpdateInfo> {
        match self {
...
mod tests {
    ...
    fn v2_with_products_roundtrip() {
        let json = r#"{"VersionMajor":2,"VersionMinor":1,"Products":{"Agent":{"Version":"2026.2.0"},"Gateway":{"Version":"latest"}}}"#;
        let status = UpdateStatus::parse(json.as_bytes()).unwrap();
        let products = status.into_products();
        assert_eq!(products.len(), 2);
        assert!(matches!(
            products[&UpdateProductKey::Gateway].version,
            VersionSpecification::Latest
        ));
        assert!(matches!(
            products[&UpdateProductKey::Agent].version,
