...
/// Currently installed version of a product, as reported in `update_status.json`.
///
/// Distinguished from [`ProductUpdateInfo`] (which carries a *requested* target version
/// for `update.json`) so that the JSON key name clearly reflects what the field means:
/// `"Version"` is the version that is on disk right now.
#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(rename_all = "PascalCase")]
...
mod tests {
    ...
    fn v2_with_products_roundtrip() {
        let json = r#"{"VersionMajor":2,"VersionMinor":0,"Products":{"Agent":{"Version":"latest"},"Gateway":{"Version":"2026.1.0"}}}"#;
    ...
    #[test]
    fn detect_version_unparsable_returns_error() {
