...
impl StringSid {
    ...
    pub fn into_ucstring(self) -> U16CString {
        self.inner
    }
}
...
impl SidAndAttributes {
    ...
    pub unsafe fn from_raw(sid_and_attributes: &Security::SID_AND_ATTRIBUTES) -> Result<Self, FromPsidErr> {
        // SAFETY: Safe safety preconditions as this function.
...
impl Sid {
    ...
    ///
    /// The SID must not be mutated through the returned PSID.
    ...
    pub fn from_well_known(sid_type: WELL_KNOWN_SID_TYPE, domain_sid: Option<&Sid>) -> anyhow::Result<Self> {
        ...
        let mut sid = unsafe { RawBuffer::alloc_zeroed(layout)? };
    ...
    pub unsafe fn from_psid(psid: Security::PSID) -> Result<Self, FromPsidErr> {
        ...
        if is_valid_sid.as_bool() {
            ...
            // On success, CopySid properly initialized the SID.
            let sid = unsafe { sid.assume_init() };

            // Caller must ensure that psid is a valid SID.
    ...
    pub fn from_string_sid(value: &StringSid) -> anyhow::Result<Self> {
        ...
        unsafe { ConvertStringSidToSidW(value.as_ucstr().as_pcwstr(), &mut psid)? };
    ...
    pub fn lookup_account(&self, system_name: Option<&U16CStr>) -> anyhow::Result<AccountWithType> {
        ...
        // If nothing wrong happens, it’s probably fine without shrinking? :shrug:
...
mod tests {
    ...
    fn create() {
        ...
        assert_eq!(sid.to_string_sid().unwrap().as_ucstr(), u16cstr!(""));
    }

    #[rstest]
    #[case(Security::WinAccountAdministratorSid)]
    #[case(Security::WinAccountCertAdminsSid)]
    #[case(Security::WinAccountDomainGuestsSid)]
    #[case(Security::WinAccountGuestSid)]
    #[case(Security::WinAccountPolicyAdminsSid)]
    ...
    #[case("")]
    #[cfg_attr(miri, ignore)]
    fn from_string_sid(#[case] input: &str) {
        ...
        assert_eq!(input_string_sid, output_string_sid);
    }

    #[test]
    #[cfg_attr(miri, ignore)]
    fn lookup_account() {
        let sid = Sid::from_well_known(Security::WinAccountAdministratorSid, None).unwrap();
        let account = sid.lookup_account(None).unwrap();
        assert_eq!(account.name, u16cstr!(""));
        assert_eq!(account.domain_name, u16cstr!(""));
    }
}
