...
pub fn is_username_valid(server_name: Option<&str>, username: &str) -> anyhow::Result<bool> {
    let server_name = server_name
        .map(U16CString::from_str)
        .transpose()
        .context("invalid server name")?;

    let username = U16CString::from_str(username).context("invalid username")?;
    ...
    let status = unsafe {
        NetUserGetInfo(
            server_name.as_deref().map_or_else(PCWSTR::null, U16CStrExt::as_pcwstr),
            username.as_pcwstr(),
            4,
...
pub fn create_profile(account_sid: &Sid, account_name: &U16CStr) -> anyhow::Result<U16CString> {
    ...
    unsafe {
        CreateProfile(
            account_string_sid.as_ucstr().as_pcwstr(),
